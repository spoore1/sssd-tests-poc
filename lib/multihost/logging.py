...
class MultihostLogger(logging.Logger):
    ...
    def colorize(self, text: str | Any, *colors: colorama.Fore | colorama.Back | colorama.Style) -> str:
        """
    ...
    def dumps(self, o) -> str:
        ...
        if isinstance(o, dict):
            ...
            for key, value in o.items():
                ...
                out += '\n'
                out += textwrap.indent(
                    f'{self.__logger.colorize(key, colorama.Fore.BLUE)}: {value}',
                    ' ' * 2
                )
        ...
        if hasattr(record, 'data'):
            for key, value in record.data.items():
                ...
                record.msg += textwrap.indent(
                    f'{self.__logger.colorize(key, colorama.Fore.MAGENTA)}: {self.dumps(value)}',
                    ' ' * 33
                )
